...
describe("XHRConnection", () => {
  describe("defaults", () => {
    it("uses method=GET as default", function() {
      ...
      req.open(token);
      expect(req._.native.open.mock.calls).toEqual([["GET", url]]);
    });

    it("uses data=null as default", function() {
      const req = new XHRConnection(url);
      req.open(token);
      expect(req._.native.send.mock.calls).toEqual([[null]]);
    });

    it("uses contentType=null as default (does not set header)", function() {
      const req = new XHRConnection(url);
      req.open(token);
      expect(req._.native.setRequestHeader.mock.calls).not.toContainEqual([
        "Content-Type",
        null
      ]);
    });
  ...
  describe("open connection", () => {
    ...
    it("open correctly with given token", function() {
      const req = new XHRConnection(url);
      req.open(token);
      expect(req._.native.setRequestHeader.mock.calls).toContainEqual([
        "Authorization",
        "Bearer " + token
      ]);
    });
    ...
    ["GET", "POST", "DELETE", "PUT"].forEach(method => {
      it(`opens connection with given method (${method})`, function() {
        ...
        req.open(token);
        expect(req._.native.open.mock.calls).toEqual([[method, url]]);
      });
    });
  });
