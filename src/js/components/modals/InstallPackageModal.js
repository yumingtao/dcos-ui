...
class InstallPackageModal
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!props.open && nextProps.open) {
      if (nextProps.isBetaPackage) {
        const packageConfig = nextProps.cosmosPackage.getConfig();

        this.setState({
          betaTermsAccepted: false,
          betaOptInProperties: BetaOptInUtil.getProperty(packageConfig),
  ...
  handleAcceptBetaTerms() {
    // Set accept terms on config model
  ...
  handleInstallPackage() {
    ...
    if (isBetaPackage && betaTermsAccepted) {
      configuration = BetaOptInUtil.acceptTerms(configuration);
    }
